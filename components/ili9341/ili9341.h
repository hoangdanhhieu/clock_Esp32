...
void lcd_send_buffer(spi_device_handle_t spi, uint16_t Xs, uint16_t Xe, uint16_t Ys, uint16_t Ye, const uint8_t *data, int len);
void lcd_data(spi_device_handle_t spi, const uint8_t *data, int len);
void fill_display(color_struct *bg_color);
void lcd_cmd(spi_device_handle_t spi, const uint8_t cmd, bool keep_cs_active);
void show_text(const uint8_t *str,
