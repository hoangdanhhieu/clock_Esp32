...
#define VL53L0X_OsDelay(...) vTaskDelay(5 / portTICK_PERIOD_MS);
...
int _I2CWrite(VL53L0X_DEV Dev, uint8_t *pdata, uint32_t count) {
    ...
    status = i2c_master_transmit(*(Dev->I2cHandle), pdata, count, i2c_time_out);
    return status;
}
...
int _I2CRead(VL53L0X_DEV Dev, uint8_t *pdata, uint32_t count) {
    int status;
    int i2c_time_out = I2C_TIME_OUT_BASE+ count* I2C_TIME_OUT_BYTE;
    status = i2c_master_receive(*(Dev->I2cHandle), pdata, count, i2c_time_out);
